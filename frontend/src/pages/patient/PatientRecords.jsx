...
const PatientRecords = ({ isOpen, onClose, patientEmail }) => {
    ...
    const fetchPatientRecords = async () => {
        ...
        try {
            ...
            const patientResponse = await axios.get(
                `http://localhost:5000/api/doctor/${doctorId}/patient/${encodeURIComponent(normalizedEmail)}`,
                { headers: { Authorization: `Bearer ${token}` } }
            );

            const [historyRes, prescriptionsRes, notesRes] = await Promise.all([
                axios.get(`http://localhost:5000/api/patient/${encodeURIComponent(normalizedEmail)}/medical-history`, {
                    headers: { Authorization: `Bearer ${token}` }
                }).catch(() => ({ data: [] })),
                axios.get(`http://localhost:5000/api/patient/${encodeURIComponent(normalizedEmail)}/prescriptions`, {
                    headers: { Authorization: `Bearer ${token}` }
                }).catch(() => ({ data: [] })),
                axios.get(`http://localhost:5000/api/patient/${encodeURIComponent(normalizedEmail)}/visit-notes`, {
                    headers: { Authorization: `Bearer ${token}` }
                }).catch(() => ({ data: [] }))
            ]);
    ...
    const handleAddPrescription = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post(
                `http://localhost:5000/api/patient/${encodeURIComponent(patientEmail)}/prescriptions`,
                {
                    ...newPrescription,
    ...
    const handleAddVisitNote = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post(
                `http://localhost:5000/api/patient/${encodeURIComponent(patientEmail)}/visit-notes`,
                {
                    ...newNote,
                    doctorId: localStorage.getItem('doctorId'),
    ...
    const handleAddVitals = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post(
                `http://localhost:5000/api/patient/${encodeURIComponent(patientEmail)}/vitals`,
                {
                    ...newVitals,
                    doctorId: localStorage.getItem('doctorId'),
