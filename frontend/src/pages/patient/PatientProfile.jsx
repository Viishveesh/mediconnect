...
const PatientProfile = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const response = await axios.get('https://mediconnect-7v1m.onrender.com/api/patient/profile', {
      ...
      if (response.data.profilePhoto) {
        setPhotoPreview(`https://mediconnect-7v1m.onrender.com/api/files/${response.data.profilePhoto}`);
      }
    } catch (error) {
  ...
  const uploadPhoto = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post('https://mediconnect-7v1m.onrender.com/api/upload', formData, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios[method]('https://mediconnect-7v1m.onrender.com/api/patient/profile', submitData, {
        headers: { Authorization: `Bearer ${token}` }
      });
